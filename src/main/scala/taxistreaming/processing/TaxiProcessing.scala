package taxistreaming.processing

import taxistreaming.utils.PointInPoly
...
object TaxiProcessing {
  ...
  def appendStartEndNeighbourhoods(df: DataFrame, sparks: SparkSession): DataFrame = {
    ...
    val lookupTable = sparks.read.json("C:\Users\Dilip\Documents\GitHub\spark-mllib-streaming\NYC_neighborhoods\nbhd.jsonl").select("name", "coord")
